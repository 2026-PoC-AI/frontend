...
export function VideoResults() {
  ...
  const results = useMemo(() => {
    ...
    if (detectedTechniquesStr && detectedTechniquesStr !== "none") {
      const techniqueNames = detectedTechniquesStr
        .split(",")
        .map((t) => t.trim());
  ...
  const getRiskColor = () => {
    if (confidence > 70) return { text: "text-red-500", bg: "bg-red-50" };

    if (confidence > 40) return { text: "text-orange-500", bg: "bg-orange-50" };
  ...
  const suspiciousFrames = results.frameAnalyses.filter(
    (f) => f.confidence > 70,
  ).length;
  ...
  return (
    <main className="relative px-6 pt-10 pb-20 max-w-6xl mx-auto">
      {/* Header */}
      <div className="text-center mb-12">
        ...
        <div
          className={`mt-4 inline-block px-6 py-3 rounded-full ${riskColors.bg} border ${riskColors.text} border-current`}
        >
      ...
      <div className="w-full max-w-5xl mx-auto animate-fade-up space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1 p-8 rounded-[24px] bg-white/60 backdrop-blur-md border border-white/60 shadow-glass-soft flex flex-col items-center justify-center text-center">
            <h3 className="text-lg font-medium text-text-sub mb-2">
              딥페이크 위험도
            </h3>

            <div className={`text-6xl font-bold ${riskColors.text}`}>
              {confidence}
              <span className="text-2xl text-text-sub/50">/100</span>
            </div>
            ...
            <div className="mt-6 text-sm text-text-sub space-y-1">
              ...
              <p
                className={
                  suspiciousFrames > 0 ? "text-orange-600 font-medium" : ""
                }
              >
          ...
          <div className="flex-[1.5] p-8 rounded-[24px] bg-white/60 backdrop-blur-md border border-white/60 shadow-glass-soft flex flex-col">
            <h3 className="text-lg font-medium text-text-sub mb-4">
              분석 정보
            </h3>
            ...
            <div className="flex-1 bg-gradient-to-br from-primary/5 to-primary/10 rounded-xl overflow-hidden flex items-center justify-center min-h-[300px] relative">
              <div className="text-center p-8">
                <div
                  className={`w-20 h-20 ${
                    results.isDeepfake ? "bg-red-100" : "bg-green-100"
              ...
              <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-2 shadow-sm">
                평균 신뢰도:{" "}
                <strong className={riskColors.text}>{confidence}%</strong>
              </div>
            </div>
